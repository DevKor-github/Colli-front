...
const ProgressCategory = () => {
  ...
  const [currentState, setCurrentState] = useState<ProgressState>('todo')
  return (
    <View w="100%">
      ...
      <BottomSheet isOpen={open} bottomSheetTitle="진행상태" setOpen={setOpen}>
        {progressConfig.map(item => (
          <Option key={item.label}>
            ...
            <Pressable onPress={() => setCurrentState(item.state)}>
              <Option.Radio checked={item.state === currentState} />
            </Pressable>
          </Option>
        ))}
      </BottomSheet>
